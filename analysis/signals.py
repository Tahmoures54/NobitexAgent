"""
Signal engine for CryptoScanner.  (v7.4 — Risk cache + return fix)

The strategy is deliberately data-driven: it works from the actual market values
provided by the data source (CoinMarketCap in the production UI), not from a
synthetic price generator.  The simulator is only used by the paper-execution
layer.

CHANGES vs v7.3
───────────────
1. `_risk()` now memoises its result on the row dict under a private key.
   Previously, `composite_strategy` (and `advanced_signal_strategy` inside it)
   called `assess_risk_details()` 6–7 times per row.  With the cache, it is
   called exactly once per row.  This is a large win because risk assessment
   builds 30+ normalised factors per call.

2. `apply_strategy_to_df` — fixed the redundant `return out if return_frame
   else out` (both branches returned the same object).  The in-place path
   now correctly returns the mutated input `df`.

3. Parallel parameter is documented as a no-op (kept for backward compat).
   pandas `.apply` already GIL-optimises the Python callable path; splitting
   it across threads did not help in profiling and was removed.

4. Added a small sanity guard so `_risk()` never raises if the caller passes
   a non-dict-like row.
"""
...
# ── Private row key used to memoise the risk call within one row ──
_RISK_CACHE_KEY = "__risk_cached_v74"
...
def _risk(row: Dict[str, Any]) -> str:
    ...
    Memoised on the row dict under `_RISK_CACHE_KEY` so that when a strategy
    fans out into sub-strategies (basic, momentum, trend, breakout), the
    expensive `assess_risk_details` is called only once per row.

    NOTE: this mutates the row dict by adding a private key.  Callers who
    reuse the same dict for multiple different market rows should not — but
    that is already true of the rest of the API (strategies read specific
    keys by name and assume stable values for the duration of the call).
    """
    try:
        cached = row.get(_RISK_CACHE_KEY)
    ...
    try:
        row[_RISK_CACHE_KEY] = result
    except (TypeError, AttributeError):
        # Non-mutable mapping (frozen dict, etc.) — just skip caching.
        pass
    return result


def _signal_from_score(score: float) -> str:
    if score >= CONFIG["strong_buy_threshold"]: return "Strong Buy"
    if score >= CONFIG["buy_threshold"]: return "Buy Signal"
    if score <= CONFIG["strong_sell_threshold"]: return "Strong Sell"
    if score <= CONFIG["sell_threshold"]: return "Sell Signal"
    return "Neutral"
...
def generate_signals(
    ...
    **kwargs,
    ...
    for r in rows:
        ...
        # Strip the private cache key so callers don't see it
        r.pop(_RISK_CACHE_KEY, None)
    return rows
