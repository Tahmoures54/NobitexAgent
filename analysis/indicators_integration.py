# analysis/indicators_integration.py  (v4.5 — Warmup cache fix + MA trend)
"""
Data Enrichment & Decision Engine v4.5
════════════════════════════════════════════════════════════════════════════
CHANGES FROM v4.4
──────────────────
- FIXED: `required_warmup` no longer relies on an unused `cfg_hash` argument
  with `@lru_cache`. That parameter was never actually passed, so the cache
  always returned the value from the very first call regardless of whether
  `INDICATOR_CONFIG` had been changed at runtime (e.g. by the settings UI).
  The warmup is now cached against a stable hash of the config's relevant
  integer fields, so a changed config invalidates the cache automatically.

- Small refactor: `_WARMUP_KEYS` names the config keys that influence warmup.
...
class DecisionMatrix:
    ...
    def validate(self) -> List[str]:
        ...
        if self.penalty_extreme != 0.0:
            errors.append(f"penalty_extreme must be 0.0, got {self.penalty_extreme}")
        return errors
