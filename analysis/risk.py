# analysis/risk.py  (v4.3 — Cached specs + OrderedDict cache + MA20_Dev fix)
"""
Multi-Engine Crypto Risk Assessment Engine v4.3
────────────────────────────────────────────────────────────────────────────
CHANGES FROM v4.2
─────────────────
1. `_build_engine_specs` is now cached with lru_cache — avoids rebuilding
   the FactorSpec dict for every row (was rebuilt O(N) per DataFrame).
2. `RiskCache` uses `collections.OrderedDict` and `popitem(last=False)` for
   O(1) eviction instead of an O(n) min-scan.
3. FIXED: `_derive_features` MA20_Dev fallback. When ATR% is missing, the
   previous fallback used percentage deviation directly (range 0..100) which
   clamped to the normalisation cap and produced distorted risk scores.
   The fallback now divides by a nominal 3% ATR so the value stays in the
   same 0..3 range as the ATR-normalised path.

CHANGES FROM v4.1
─────────────────
1. ADDED MA20/MA50 factors to 'trend' engine:
   - ma20_dev:   price deviation from MA20 (normalized by ATR%)
   - ma_cross:   MA20/MA50 separation (normalized percentage)
2. These factors are optional; if MA20/MA50 columns are absent, engine
   works as before.
3. Added corresponding entries to FEATURE_META_DIRECTION and FEATURE_NORM_RANGES.
4. Extended _derive_features to compute MA20_Dev and MA_Cross if missing.
"""
...
FEATURE_META_DIRECTION: Dict[str, int] = {
    ...
    "MA20_Dev":              +1,   # deviation from MA20 (risk if extreme)
    "MA_Cross":              +1,   # separation between MA20 and MA50 (risk if large)
}
...
FEATURE_NORM_RANGES: Dict[str, Tuple[float, float]] = {
    ...
    "MA20_Dev":      (0.0, 3.0),   # normalized by ATR, typical 0..3
    "MA_Cross":      (0.0, 2.0),   # percentage separation, typical 0..2%
}
...
def _derive_features(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if ma20 is not None and price is not None and ma20 > 0:
        ...
        if atr_pct is not None and atr_pct > 0.05:
            # Normalised by observed ATR — same scale as the norm range 0..3
...
# ════════════════════════════════════════════════════════════
# RISK CACHE (v4.3 — OrderedDict for O(1) eviction)
# ════════════════════════════════════════════════════════════
...
class RiskCache:
    ...
    v4.3: switched from a plain dict + O(n) min-scan to
    `collections.OrderedDict` + `popitem(last=False)` so eviction is O(1)
    even at the 10_000-entry cap.
    """
