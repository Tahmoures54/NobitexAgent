...
Version: 2.3 — Admin Secret Activation

Changes vs 2.2:
  - Added ADMIN_SECRET_CODE constant.
  - Added activate_admin_plan() for permanent admin activation.
  - Modified _normalize() to never expire "admin" plan.
  - Modified verify_and_activate_code() to check admin secret first.
"""
...
# ══════════════════════════════════════════════════════════════
# Module-level lock — تمام عملیات I/O از اینجا رد می‌شود
# ══════════════════════════════════════════════════════════════
...
_REPLACE_RETRY_DELAY: float = 0.05  # ثانیه

# 🔑 رمز مخصوص ادمین برای فعال‌سازی بدون تراکنش
ADMIN_SECRET_CODE: str = "505506+1Tp"
...
def verify_activation_code_signature(code: str, secret_key: bytes) -> Tuple[bool, str, dict]:
    try:
...
def _normalize(
    ...
    *,
    ...
    # اگر پلن admin باشد، هرگز منقضی نمی‌شود
    if s.get("plan") == "admin":
        # ادمین همیشه فعال است و انقضا ندارد
        s["plan_expiry"] = None
...
def activate_admin_plan(device_id: Optional[str] = None) -> None:
    """
    فعال‌سازی پلن ادمین (دائمی، بدون نیاز به تراکنش).
    """
    did = device_id or get_device_id()
    with _STATUS_LOCK:
        st = _read_raw(did)
...
# ══════════════════════════════════════════════════════════════
# Activation Code Verification (includes admin secret)
# ══════════════════════════════════════════════════════════════
...
def verify_and_activate_code(
    ...
    """
    بررسی کد فعال‌سازی.
    - اگر کد برابر ADMIN_SECRET_CODE باشد، پلن ادمین فعال می‌شود.
    - در غیر این صورت، کد امضاشده بررسی و پلن مربوطه فعال می‌شود.
    """
    code = (code or "").strip()
    if not code:
