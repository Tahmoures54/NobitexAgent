"""Market regime detector for adaptive aggression (v2.1).
...
class RegimeDetector:
    ...
    # ────────────────────────────────────────────────────
    # NEW: Strong movers — coins that move against a bad tape
    # ────────────────────────────────────────────────────
    ...
    def _trade_stats(self, tracker: Any) -> Dict[str, float]:
        ...
        if tracker is None:
            return out
        try:
            trades = tracker.get_closed_trades() if hasattr(tracker, "get_closed_trades") else []
        except Exception:
            trades = []
        if not trades:
            return out
        ...
        wins = 0
        for t in recent:
            pnl = safe_float(t.get("pnl_pct") or t.get("profit_pct") or 0.0) or 0.0
            if pnl > 0:
                wins += 1
        ...
        try:
            ...
            cash = float(getattr(tracker, "cash", 0.0) or 0.0)
            if peak > 0:
                out["drawdown_pct"] = max(0.0, (peak - cash) / peak * 100.0)
        except Exception:
            pass
        return out
