...
def backtest_status(user:User=Depends(get_current_user),db:Session=Depends(get_db)):
    ...
    snapshots=(db.query(ScanSnapshot).filter(ScanSnapshot.created_at>=cutoff)
               .order_by(ScanSnapshot.created_at.asc()).all())
    ...
    for snap in snapshots:
        try:
            ...
            if isinstance(payload,list):
                global_rows += sum(1 for row in payload if isinstance(row,dict) and row.get("GlobalPriceUSD") not in (None,"",0))
        except (TypeError,ValueError):
            continue

    last_age=age_seconds(last)
    if not snapshots: status="WAITING"
    elif last_age is not None and last_age <= max(900, settings.scan_interval_minutes*60*3): status="COLLECTING"
    else: status="STALE"
