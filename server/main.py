...
# ── Safe fallbacks (FastAPI requires non-empty title) ──────
APP_TITLE = (settings.app_name or "").strip() or "Nobitex Agent"
APP_VERSION = (settings.app_version or "").strip() or "1.0.0"
...
app = FastAPI(
    ...
    version=APP_VERSION,
    description=(
        "Nobitex scanner, paper trading, backtesting and guarded "
...
if WEB_DIR.exists():
    ...
    logger.warning(
        "Frontend directory '%s' not found — API-only mode.", WEB_DIR
    )
